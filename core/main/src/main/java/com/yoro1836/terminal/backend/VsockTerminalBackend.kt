...
import com.yoro1836.terminal.backend.avf.AvfApi
import com.yoro1836.terminal.backend.avf.AvfTerminalSize
import com.yoro1836.terminal.backend.avf.AvfConsoleRelay
import com.yoro1836.terminal.backend.avf.AvfUiState
import com.yoro1836.terminal.backend.avf.AvfVmManager
...
class VsockTerminalBackend(
    ...
    private fun runConnection() {
        ...
        try {
            // Any tab can bootstrap the VM; it outlives every tab.
            val machine = AvfVmManager.ensureRunning(appContext) { status ->
                AvfUiState.loading(status)
            }
            // rvshd starts with multi-user.target; give the guest time to
            // reach it before giving up.
            var lastError: Throwable? = null
            repeat(VSHD_CONNECT_ATTEMPTS) {
                if (pfd != null || closed.get()) return@repeat
                try {
                    pfd = AvfApi.connectVsock(machine, VSHD_PORT)
                } catch (error: Throwable) {
                    lastError = error
                    try {
                        Thread.sleep(VSHD_CONNECT_RETRY_MS)
                    } catch (_: InterruptedException) {
                        Thread.currentThread().interrupt()
                    }
                }
            }
            pfd = pfd ?: throw (lastError ?: IllegalStateException("rvshd is unreachable"))
            livePfd = pfd
    ...
    private companion object {
        ...
        const val VSHD_CONNECT_ATTEMPTS = 300
        const val VSHD_CONNECT_RETRY_MS = 2_000L
