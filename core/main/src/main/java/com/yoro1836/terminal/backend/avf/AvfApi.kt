...
internal object AvfApi {
    ...
    fun getOrCreate(manager: Any, name: String, config: Any): Any {
        val vm = try {
            invoke(manager, "getOrCreate", name, config)
                ?: error("VirtualMachineManager.getOrCreate returned null")
        } catch (error: Throwable) {
            // A dirty power-loss can persist an instance dir whose config
            // cannot be loaded; recreate it through the public API.
            android.util.Log.w("AvfApi", "Recreating unusable VM instance '$name'", error)
            val deleteError = runCatching { invoke(manager, "delete", name) }.exceptionOrNull()
            if (deleteError != null) {
                android.util.Log.w("AvfApi", "Deleting stale VM instance failed", deleteError)
            }
